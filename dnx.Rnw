...
Model izračuna pričakovano število palčk za cele vrečke $121.5$, standardni odklon $\sigma =47.5$ in napako $err _{mcmc} = \pm 1.3$. Za spuščene vrečke dobimo vrednosti $\mu = 303.7$, $\sigma = 132.6$ in napako $err _{mcmc} = \pm 5.8$.

Na slikah \ref{fig1}-\ref{fig4} so podane (od zgoraj navzdol) ocene parametrov, traceplot, napovedna aposteriorna porazdelitev in pripadajoči (normalizirani) histogram za dolžine palčk.

\begin{minipage}[t]{0.48\linewidth}
<<label=fig1, fig=T,width=6,height=6>>=
...
<<label=fig2, fig=T,width=6,height=6>>=
library("rstan") # observe startup messages
library(ggplot2)
library(gridExtra)
rstan_options(auto_write = TRUE)
...
<<label=fig3, fig=T,width=6,height=6>>=
library("rstan") # observe startup messages
library(ggplot2)
library(gridExtra)
rstan_options(auto_write = TRUE)
...
<<label=fig4, fig=T,width=6,height=6>>=
library("rstan") # observe startup messages
library(ggplot2)
library(gridExtra)
rstan_options(auto_write = TRUE)
...
\section{Diskusija}
Pričakovano število zlomljenih palčk se je po padcu vrečke bistveno povečalo, kar ni presenetljivo. Tudi povečanje standardnega odklona ni presenetljivo. Glede na to, da smo imeli na voljo le dva vzorca pred padcem in dva vzorca po padcu, še vedno menimo, da kljub nizki napaki $err_{mcmc}$ model ni robusten. 
