...
p1 = ggplot() + geom_density(aes(x=extract(output1)$y_pred))+xlim(0, 1000) + 
...
p1 = ggplot() + geom_density(aes(x=extract(output2)$y_pred))+xlim(0, 1000) + 
  xlab("Stevilo palck") + ylab("Gostota") + 
  theme_gray(base_size=13)
p2 = plot(output2)
...
\vspace{-.65cm}
\captionof{figure}{Dolžine prva vrečka.}
\label{fig3}
\vspace{.3cm}
\includegraphics[width = \linewidth]{./dnx-FIG4}
\vspace{-.65cm}
\captionof{figure}{Dolžine druga vrečka.}
\label{fig4}
\vspace{.3cm}
...
\hfill
\begin{minipage}[t]{0.48\linewidth}
\includegraphics[width = \linewidth]{./dnx-FIG5}
\vspace{-.65cm}
\captionof{figure}{Dolžine tretja vrečka.}
\label{fig5}
\vspace{.3cm}
\includegraphics[width = \linewidth]{./dnx-FIG6}
\vspace{-.65cm}
\captionof{figure}{Dolžine četrta vrečka.}
\label{fig6}
\vspace{.3cm}
\end{minipage}
